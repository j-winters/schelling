'''
title           : app.py
description     : Schelling segregation model using Python and Streamlit. 
author          : James Winters
usage           : python -m streamlit run schelling.py
...
class Schelling:
    def __init__(self, size, empty_ratio, similarity_threshold, n_neighbors):
        ...
        # Calculate ratio of attributes (-1, 1) and empty houses (0)
    ...
    def run(self):
        for (row, col), value in np.ndenumerate(self.city):
            attribute = self.city[row, col]
            # if attribute is not an empty house, then
            if attribute != 0:
                ...
                if neighborhood_size != n_empty_houses + 1:
                    n_similar = len(np.where(neighborhood == attribute)[0]) - 1
